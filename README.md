# Technical Writing Portfolio - Vanitha Nanaiah
![GitHub Pages](https://img.shields.io/badge/Hosted-GitHub%20Pages-blue)
![Technical Writing](https://img.shields.io/badge/Role-Technical%20Writer-green)
![Markdown](https://img.shields.io/badge/Format-Markdown-red)
![HTML/CSS](https://img.shields.io/badge/Includes-HTML%20%26%20CSS-orange)
![API Documentation](https://img.shields.io/badge/Focus-API%20Docs-yellow)
![Developer Docs](https://img.shields.io/badge/Developer%20Experience-DX-9cf)

Welcome to my Technical Writer Portfolio. This repository showcases my work across API documentation, tutorials, user guides, and engineering content.

### **Live Portfolio Website**

## Repository Structure
/index.html -> Landing page(portfolio website)\
/style.css -> Website Styling\
/style-guide.md -> My documentation style guide\
/contact.html -> My contact details\
/api-documentation\
/user-guide

## Skills Demonstrated
- Technical writing (API docs, user guides, tutorials)
- Backend engineering: C#, .NET, SQL
- HTML, CSS, XML
- Markdown, DITA XML, Confluence, JIRA, Swagger
- GIT, GitHub, GitHub Actions